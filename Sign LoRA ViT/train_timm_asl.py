...
from utils.dataloader_sign import signDataloader
from utils.dataloader_sign_WLASL2000 import signDataloader_WLASL2000
...
def train(epoch,trainset):
    ...
    logging.info(f"\n\nEPOCH: {epoch}, LOSS : {loss:.4f}, LR: {this_lr:.2e}")
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("-data_path",type=str, default='../data/gloss518/train')    
    parser.add_argument("-use_WLASL2000",type=str, default=False)    
    ...
    trainset, valset, testset = signDataloader(cfg)
    ...
    for epoch in range(1, cfg.epochs+1):
        ...
        if epoch%1==0:
            ...
            if save_model:
                if cfg.train_type == "lora":
                    net.module.save_lora_parameters(ckpt_path.replace(".pt", "_epoch"+str(epoch)+".safetensors"))
                else:
                    torch.save(net.state_dict(), ckpt_path.replace(".pt", "_best.pt"))
            eval(epoch,testset,datatype='test')
            logging.info(f"BEST VAL: {result.best_val_result:.4f}, TEST: {result.test_auc:.4f}, EPOCH: {(result.best_epoch):3}")
